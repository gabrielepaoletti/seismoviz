...
class CatalogPlotter:
    ...
    def plot_map(self, highlight_mag: int=None, color_by: str=None, cmap: str='jet', title: str=None, size: float=0.5, color: str='lightgrey',
        ...
        if color_by:
            ...
            if color_by == 'time':
                ...
                scatter = self.mp.scatter(x=self.catalog.lon, y=self.catalog.lat, c=color_numeric, s=size, cmap=cmap, edgecolor=edgecolor,
                                        linewidth=0.25, alpha=alpha, label=legend)
                
                cbar = plt.colorbar(scatter, ax=self.mp.ax, orientation='horizontal', pad=0.07, shrink=0.6, aspect=40)
                ...
                scatter = self.mp.scatter(x=self.catalog.lon, y=self.catalog.lat, c=color, s=size, cmap=cmap, edgecolor=edgecolor,
                                        linewidth=0.25, alpha=alpha, label=legend)
            ...
            self.mp.scatter(x=self.catalog.lon, y=self.catalog.lat, c=color, s=size, edgecolor=edgecolor, 
                            linewidth=0.25, alpha=alpha, label=legend)
    ...
    def plot_magnitude_time(self, save_figure: bool=False, save_name: str='map', save_extension: str='jpg') -> None:
        """
        ...
        ax.scatter(self.catalog.time, self.catalog.mag, c='lightgrey', s=self.catalog.mag, edgecolor='grey', linewidth=0.25, alpha=0.5, zorder=10)
    ...
    def plot_event_timeline(self, save_figure: bool=False, save_name: str='map', save_extension: str='jpg') -> None:
        """
    ...
    def plot_attribute_distributions(self, save_figure: bool=False, save_name: str='map', save_extension: str='jpg') -> None:
        """
        ...
        for i, (attribute, label) in enumerate(labels.items()):
            ...
            ax[row, col].hist(self.catalog[attribute], bins=50, color='silver', edgecolor='black', linewidth=0.5)
        ...
        if save_figure:
            self.bp._PlotMixin().save_figure(fig, save_name, save_extension)
